# The Long Hunt – Automatic Raid Mode (cfgGameplay switcher)
...
```text
/home/d3nd4n/dayz_raid/
├── raid_mode.py
├── cfggameplay_raid_on.json
└── cfggameplay_raid_off.json
```

If your username is not `d3nd4n`, just adjust the paths in `raid_mode.py` and in your `crontab` later.
...
> ⚠️ If you forget to copy the full file and only use the small example, your server will NOT start. Always use the full original content with only those 2 booleans changed.
...
Have fun raiding only when **The Long Hunt** says it’s time. 🦌🔨
